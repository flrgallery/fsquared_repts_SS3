...
performance(f0, statistics=statistics["PBlim"])[year %in% seq(iy, fy, by=5),
...
dt0 <- performance(f0, statistics=statistics["IACB"])[year %in% iy:fy,
...
tune <- tunebisect(om, control=arule, args=mseargs,
  ...
  statistic=statistics["PBlim"], prob=0.05, tol=0.005, years=pys)

# PLOT
plot(om, tune)

# COMPUTE performance
performance(tune) <- performance(tune, statistics=statistics, type="arule", run="tune")
